...
| **2 - Foundational** | Certificates are managed, but not according to industry standards and regulations. Inventory of certificates is not maintained. |
...
| **4 - Managed** | Certificate profiles, attributes, cryptographic algorithms and parameters, and tooling are properly documented and applied in the organization. Certificate management procedures are well designed and followed to maintain an up-to-date inventory of certificates, including their state and location. |
...
Known certificates include the certificates the organization issues or holds, and the certificates configured as trust anchors in its systems. The trust anchors recorded here are the deployed counterpart of the accepted set defined in the certificate validation policy, and recording them is what makes it possible to tell whether the deployed state still matches the policy.
...
- Examine the trust stores of systems and devices to discover trust anchors that are not part of the accepted set
