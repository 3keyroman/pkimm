...
| Certificate management (Management) | Removed the "Certificate cipher suites are documented" requirement. Cipher suites are a protocol-level concept primarily associated with TLS and similar protocols; certificates themselves do not define or negotiate them. Including this requirement in Certificate management mixed cryptographic governance with certificate lifecycle management. Cryptographic algorithm and protocol governance is now handled centrally in the new Cryptography category, so Certificate management focuses on certificate lifecycle topics. Clarified that the certificate inventory and the discovery process cover the trust anchors an organisation relies on, which are the deployed counterpart of the accepted set defined by the new certificate validation policy. | <span class="badge text-bg-success">Evolving</span> |
| Policies and documentation (Governance) | Added the certificate validation policy requirement. The category previously covered the documents an issuing organisation publishes, but no dedicated requirement covered the rules by which an organisation accepts the certificates it is presented with. | <span class="badge text-bg-success">Evolving</span> |
...
| Cryptography | Cryptographic algorithms and parameters are documented and approved | Added. Assessors evaluate whether the organisation has documented and formally approved the cryptographic algorithms and parameters in use across its PKI. | <span class="badge text-bg-success">Evolving</span> |
...
| Certificate management | Inventory of certificates is documented | Renamed from "Inventory of issued certificates is documented" and clarified to state that known certificates include the trust anchors an organisation relies on. The guidance already described the inventory as covering all known certificates; the previous title was the only one in the model scoped to issued certificates and read as not applicable to an organisation that issues none. The identifier `cert-inventory` and the weight are unchanged. | <span class="badge text-bg-warning">Clarification or Guidance</span> |
| Certificate management | Certificate discovery process is documented | Clarified that discovery covers the trust stores of systems and devices, alongside the network and file system locations already described. The guidance already named unauthorised certification authorities as the risk discovery addresses. | <span class="badge text-bg-warning">Clarification or Guidance</span> |
| Policies and documentation | Certificate validation policy is documented and applied | Added. Assessors evaluate whether the organisation has documented how certificates are validated — the trust anchors accepted and the approval required to change them, path construction and validation, name constraint and key usage enforcement, required or rejected policy identifiers, verification that the certificate identifies the subject the relying system expects, the revocation mechanisms used and the status freshness that is acceptable, and the behaviour when revocation status cannot be determined — and whether deployed trust stores and validation settings match those rules. Approved algorithms and parameters remain governed by the Cryptography category. | <span class="badge text-bg-success">Evolving</span> |
